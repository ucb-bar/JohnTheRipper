/* RoCC-accelerated SHA3-256 cracker patch for JtR.
...
#if FMT_EXTERNS_H
extern struct fmt_main fmt_rawSHA3_256_rocc;
#elif FMT_REGISTERS_H
john_register_one(&fmt_rawSHA3_256_rocc);
#else
...
#define FORMAT_LABEL			"Raw-SHA3-256-rocc"
#define FORMAT_NAME			""
...
static int crypt_all(int *pcount, struct db_salt *salt)
{
	...
	for (index = 0; index < count; index++) {
		__asm__ __volatile__ (
			".insn r CUSTOM_2, 0x3, 0x0, zero, %0, %1\n"
			".insn r CUSTOM_2, 0x2, 0x1, zero, %2, zero\n"
			:
			: "r" (saved_key[index]),
			  "r" (crypt_out[index]),
			  "r" (saved_len[index])
...
struct fmt_main fmt_rawSHA3_256_rocc = {
	{
		FORMAT_LABEL,
		FORMAT_NAME,
...
#warning ": format disabled on unsupported architecture"
#endif
